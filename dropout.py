...
class ExampleTiedDropout(torch.nn.Module):
    ...
    def __init__(self, p_fixed = 0.2, p_mem = 0.1, drop_mode = "train"):
    ...
    def forward(self, X, idx, epoch = 0):
        ...
        if self.drop_mode == "train":
            ...
            mask = torch.zeros_like(X[..., 0, 0]).cpu()
            shape = X.shape[1]
            ...
            if epoch > 0:
                ...
                if self.mask_tensor is None:
                    self.mask_tensor = torch.zeros(self.max_id, X.shape[1]).cuda()
                #assign mask at positions given by idx
                self.mask_tensor[idx] = mask.cuda()
            
            # Apply the mask to the input tensor
            X = X * mask[..., None, None].cuda()
